...
$sql_supplier = "SELECT username, password FROM supplier WHERE kode_supplier = ?";
...
if ($result_supplier->num_rows === 0) {
    echo "<script>alert('Data supplier tidak ditemukan!'); window.location.href='akun.php';</script>";
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($new_username) || empty($new_password)) {
        ...
        $sql_update = "UPDATE supplier SET username = ?, password = ? WHERE kode_supplier = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $new_username, $new_password, $supplier_id);

        if ($stmt_update->execute()) {
            echo "<script>alert('Data akun berhasil diperbarui!'); window.location.href='akun.php';</script>";
        } else {
        ...
        <div class="account-container">
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <input type="text" id="username" name="username" value="<?php echo htmlspecialchars($data_supplier['username']); ?>" required>
                </div>
                <div class="form-group">
                    <label for="password">Password</label>
                    <input type="text" id="password" name="password" value="<?php echo htmlspecialchars($data_supplier['password']); ?>" required>
                </div>
